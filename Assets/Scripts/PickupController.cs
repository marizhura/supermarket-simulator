...
public class PickupController : MonoBehaviour
{
    ...
    void TryInteract()
    {
        ...
        if (playerHands.CurrentItem == null)
        {
            ...
            if (item != null)
            {
                Customer waitingCustomer = Customer.WaitingCustomer;
                if (waitingCustomer != null && waitingCustomer.cashPoint != null)
                {
                    if (item.transform.IsChildOf(waitingCustomer.cashPoint))
                    {
                        return;
                    }
                }
